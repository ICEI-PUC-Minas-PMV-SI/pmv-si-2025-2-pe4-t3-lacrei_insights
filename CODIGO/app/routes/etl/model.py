...
def _rodar_etl_model(conn):
    ...
    # -------------------------------------------------------------------------
    # 2) DIM CANCELLATION
    #    Removed: cancellations are denormalized into the appointments fact
    #    (cancellation_created_at, cancellation_reason). The canonical
    #    cancellation dimension was intentionally removed to avoid storing
    #    source ids in dims and to simplify the ETL. If needed, a dim or
    #    mapping table can be reintroduced later.
    # -------------------------------------------------------------------------
    ...
    conn.execute(text("""
        ...
        WHERE NOT EXISTS (
            ...
            WHERE f.date_id = j.date_id
              ...
              AND COALESCE(f.cancellation_created_at, 'epoch') = COALESCE(j.cancellation_created_at, 'epoch')
              AND COALESCE(f.cancellation_reason, '') = COALESCE(j.cancellation_reason, '')
