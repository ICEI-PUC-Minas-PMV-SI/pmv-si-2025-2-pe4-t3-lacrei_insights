...
def login():
    ...
    if request.method == 'POST':
        ...
        if username == 'admin' and password == '123456':
            ...
            # Força o redirecionamento usando URL absoluta para evitar cache
...
def proteger_apidocs(app):
    ...
    def verificar_apidocs():
        ...
        if caminho.startswith('/apidocs') or caminho.startswith('/dashboard') or caminho.startswith('/api') or caminho.startswith('/upload'):
    ...
    def adicionar_cabecalhos_cache(response):
        if request.path.startswith('/apidocs') or request.path.startswith('/login') or request.path.startswith('/dashboard'):
            response.headers['Cache-Control'] = 'no-store, no-cache, must-revalidate, post-check=0, pre-check=0, max-age=0'
            response.headers['Pragma'] = 'no-cache'
